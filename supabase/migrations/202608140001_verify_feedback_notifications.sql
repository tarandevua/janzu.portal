create temporary table task_001_feedback_notification_matches
as
with candidates as (
  select
    ...
    abs(extract(epoch from notifications.created_at - session_feedback.submitted_at)) as distance_seconds
  ...
  select
    candidates.*,
    row_number() over (
      partition by candidates.notification_id
      order by candidates.distance_seconds, candidates.feedback_id
    ) as notification_rank,
    row_number() over (
      partition by candidates.feedback_id
      order by candidates.distance_seconds, candidates.notification_id
    ) as feedback_rank,
    count(*) over (
      partition by candidates.notification_id, candidates.distance_seconds
    ) as notification_distance_ties,
    count(*) over (
      partition by candidates.feedback_id, candidates.distance_seconds
    ) as feedback_distance_ties
  from candidates
)
select
  ranked_candidates.notification_id,
  ranked_candidates.feedback_id,
  ranked_candidates.participant_name,
  ranked_candidates.session_date,
  ranked_candidates.rating
from ranked_candidates
where ranked_candidates.notification_rank = 1
  and ranked_candidates.feedback_rank = 1
  and ranked_candidates.notification_distance_ties = 1
  and ranked_candidates.feedback_distance_ties = 1;

update public.notifications
set feedback_id = null,
    participant_name = null,
    feedback_session_date = null,
    feedback_rating = null,
    title = 'Session feedback received',
    body = 'Feedback was received for one of your sessions.',
...
where type = 'feedback_received'
  ...
  and not exists (
    select 1
    from task_001_feedback_notification_matches
    where task_001_feedback_notification_matches.notification_id = notifications.id
      and task_001_feedback_notification_matches.feedback_id = notifications.feedback_id
  );

update public.notifications
set feedback_id = task_001_feedback_notification_matches.feedback_id,
    participant_name = task_001_feedback_notification_matches.participant_name,
    feedback_session_date = task_001_feedback_notification_matches.session_date,
    feedback_rating = task_001_feedback_notification_matches.rating,
    title = 'Feedback from ' || task_001_feedback_notification_matches.participant_name,
    body = task_001_feedback_notification_matches.participant_name
      || ' submitted feedback for your session on '
      || to_char(task_001_feedback_notification_matches.session_date, 'YYYY-MM-DD')
      || '. Rating: ' || task_001_feedback_notification_matches.rating || '/5.',
    href = '/dashboard/feedback?feedbackId='
      || task_001_feedback_notification_matches.feedback_id,
    updated_at = now()
from task_001_feedback_notification_matches
where notifications.id = task_001_feedback_notification_matches.notification_id;

create unique index if not exists notifications_feedback_received_unique_idx
on public.notifications(feedback_id)
where type = 'feedback_received' and feedback_id is not null;

drop table task_001_feedback_notification_matches;
