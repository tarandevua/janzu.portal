...
add column if not exists participant_preferred_language text;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'session_feedback_participant_preferred_language_check'
  ) then
    alter table public.session_feedback
    add constraint session_feedback_participant_preferred_language_check
    check (participant_preferred_language is null or participant_preferred_language in ('en', 'es'));
  end if;
end $$;
...
create or replace function public.submit_session_feedback(
  ...
  feedback_participant_preferred_language text,
...
begin
  ...
  set participant_email = lower(nullif(trim(feedback_participant_email), '')),
      participant_preferred_language = case
        when feedback_participant_preferred_language in ('en', 'es')
          then feedback_participant_preferred_language
        else null
      end,
