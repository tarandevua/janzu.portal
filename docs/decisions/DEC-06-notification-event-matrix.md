...
- Amended: 2026-08-25 for TASK-401 training-history review events
- Implements: TASK-501
...
| Training history submitted | `training_history.submitted` after a new structured training claim and its audit event commit. | The Trainee's active assigned Instructor at submission time. An unassigned claim creates no broadcast notification. | Mandatory in-app only. | Training record ID, audit event ID, Trainee user ID and display name, level, active assignment ID, and event timestamp. Exclude evidence, notes, contact data, and cohort details. | `/{locale}/dashboard/training?traineeId={traineeUserId}&recordId={recordId}` | `training_history.submitted:{trainingAuditId}:{recipientUserId}` |
| Training history corrected | `training_history.corrected` after a Trainee corrects and resubmits a structured training claim and its audit event commits. | The Trainee's active assigned Instructor at correction time. Former Instructors and Administrators do not receive a broadcast. | Mandatory in-app only. | Training record ID, correction audit event ID, Trainee user ID and display name, level, active assignment ID, and event timestamp. Exclude previous/resulting snapshots, evidence, notes, correction reasons, contact data, and cohort details. | `/{locale}/dashboard/training?traineeId={traineeUserId}&recordId={recordId}` | `training_history.corrected:{trainingAuditId}:{recipientUserId}` |
